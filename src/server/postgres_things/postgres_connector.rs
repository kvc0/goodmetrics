...
impl PostgresConnector {
    pub async fn new(connection_string: String) -> Result<PostgresConnector, SinkError> {
        ...
        let (client, connection_handle) =
            new_connection(&connection_string, connection_is_alive.clone()).await?;
    ...
    async fn get_connection<'a>(&'a mut self) -> Result<&'a mut Client, SinkError> {
        ...
        {
            true => Ok(&mut self.client),
            false => {
                let (client, connection_handle) =
                    new_connection(&self.connection_string, self.connection_is_alive.clone())
                        .await?;
                self.client = client;
...
async fn new_connection(
    ...
    connection_is_alive: Arc<AtomicBool>,
) -> Result<(Client, JoinHandle<()>), SinkError> {
    ...
    match connection_future.await {
        Ok(connection_pair) => {
            ...
            let connection_handle =
                tokio::spawn(async move { run_connection(connection, connection_is_alive).await });

            Ok((client, connection_handle))
        }
        Err(err) => Err(SinkError::DescribedError(DescribedError {
